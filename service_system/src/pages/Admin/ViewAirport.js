...
export default function ViewAirports(props) {
  ...
  const inputFilter = (value) => {
    ...
    setFiltered(
      airports
        .filter((airport) =>
          airport.time_zone.toLowerCase().includes(time.toLowerCase())
        )
        .filter((airport) =>
          airport.airport_id.toLowerCase().includes(value.toLowerCase())
        )
    );
  ...
  const handleChange = (value) => {
    ...
    setFiltered(
      airports
        .filter((airport) =>
          airport.airport_id.toLowerCase().includes(id.toLowerCase())
        )
        .filter((airport) =>
          airport.time_zone.toLowerCase().includes(value.toLowerCase())
        )
    );
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: {
        compare: (a, b) =>
          a.total_departing_flights - b.total_departing_flights,
        multiple: 1,
      },
    },
    {
      ...
      sorter: {
        compare: (a, b) =>
          a.avg_departing_flight_cost - b.avg_departing_flight_cost,
        multiple: 1,
      },
    },
  ];
  ...
  return (
    <Layout style={{ minHeight: "100vh" }}>
      <Content style={{ margin: "24px 24px 24px", background: "white" }}>
        ...
        >
          <Col xs={22} sm={20} md={20} lg={16} xl={16} xxl={15}>
            <Row justify="center" align="middle" gutter={[24, 24]}>
              <Col span={24} align="middle">
                <h2>Now logged in as {location.state.email}</h2>
                <h1 className="heading">View Airports</h1>
              </Col>
